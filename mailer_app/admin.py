...
from .models import ContactList, Contact, EmailTemplate, Campaign, CampaignSendLog, Settings
...
class ContactAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {'fields': ('email', 'first_name', 'last_name', 'subscribed')}),
        ('Optional Details', {'classes': ('collapse',), 'fields': ('company', 'job_title', 'contact_list', 'custom_fields')}),
        ('Metadata', {'classes': ('collapse',), 'fields': ('id', 'unsubscribe_token', 'created_at', 'updated_at')}),
    )
...
class CampaignAdmin(admin.ModelAdmin):
    ...
    list_filter = ('status', 'email_template', 'contact_lists', 'scheduled_at', 'sent_at')
    ...
    fieldsets = (
        (None, {'fields': ('name', 'email_template', 'contact_lists', 'status', 'scheduled_at')}),
        ('Statistics', {'classes': ('collapse',), 'fields': ('total_recipients', 'successfully_sent', 'failed_to_send', 'sent_at')}),
        ('Metadata', {'classes': ('collapse',), 'fields': ('id', 'created_at', 'updated_at')}),
    )
...
class CampaignSendLogAdmin(admin.ModelAdmin):
    list_display = ('campaign', 'contact_email_display', 'status', 'sent_at', 'opened_at', 'clicked_at', 'message_id')
    list_filter = ('status', 'campaign__name', 'sent_at')
    search_fields = ('email_address', 'campaign__name', 'message_id', 'error_message')
    readonly_fields = ('campaign', 'contact', 'email_address', 'sent_at', 'status', 'message_id', 'error_message', 'opened_at', 'clicked_at')

    def contact_email_display(self, obj):
        return obj.contact.email if obj.contact else obj.email_address
...
class SettingsAdmin(admin.ModelAdmin):
    list_display = ('sender_email', 'company_name', 'updated_at')
